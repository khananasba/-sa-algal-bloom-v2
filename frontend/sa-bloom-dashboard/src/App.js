...
const PRIVACY_TEXT="This demonstration prototype does not intentionally collect user analytics for marketing, advertising, profiling, lead generation, retargeting, or behavioural tracking. The site does not require user registration and does not request personal information from users. Basic technical logs may be generated by hosting or security infrastructure for operational, performance, availability, or security purposes only. These logs are not used by PromaSecure/Promatas for marketing purposes.";
const INFO_SECTIONS=[
{title:"Demonstration prototype only",body:"This dashboard is a non-commercial demonstration prototype developed to show how publicly available environmental, water quality, satellite, telemetry and observational datasets could be brought together for situational awareness. It is not an official SA Government system, not an operational monitoring tool, and must not be used for public health, environmental, fisheries, aquaculture, commercial, legal, or emergency decision-making. Please refer to official SA Government, PIRSA/SARDI and SA Health channels for current advice."},
{title:"Data acknowledgement",body:"This demonstration dashboard uses or references publicly available information and open datasets from Australian and South Australian Government sources, including SA Government algal bloom information, PIRSA/SARDI algal bloom updates, the SA Algal Bloom Water Testing Open Data Dashboard, SA Health water quality alerts, data.sa.gov.au, data.gov.au, and other relevant environmental, hydrological, meteorological, satellite and spatial datasets where applicable. Public datasets are acknowledged to their respective custodians. Where SA Government open data is used, attribution should follow the required format: Organisation name, South Australia, dataset title, date sourced, dataset URL. Use of public data does not imply endorsement, sponsorship, approval, or validation by the Government of South Australia, the Australian Government, PIRSA, SARDI, SA Health, BoM, or any other data custodian."},
{title:"Important limitations",body:"The dashboard is intended to support discussion, demonstration and stakeholder engagement only. Data may be incomplete, delayed, transformed, summarised, manually loaded, or sourced from public datasets that are updated on different schedules. Satellite chlorophyll-a imagery can indicate algal biomass or bloom movement but cannot confirm the species present. Water testing and laboratory results may take several days and may not represent current beach or waterway conditions at the time of viewing. Users should always verify current conditions and official advice through the relevant government agency before making decisions."},
{title:"Non-commercial use statement",body:"This prototype is provided for demonstration, evaluation and discussion purposes only. It is not being offered as a commercial product, paid service, official advisory service, compliance tool, or replacement for government monitoring systems. No public dataset is being resold, sublicensed, or represented as proprietary PromaSecure/Promatas data. Any future commercial use, operational deployment, API integration, or customer-facing service would require confirmation of dataset licences, source-specific terms of use, attribution requirements, data quality constraints and any required permissions from relevant data custodians."},
];
...
{layer==='cellcounts'&&(<div className="data-disclaimer-banner">Data sourced from the <a href="https://experience.arcgis.com/experience/5f0d6b22301a47bf91d198cabb030670" target="_blank" rel="noopener noreferrer">SA Government Algal Bloom Water Testing Open Data Dashboard</a><button className="info-btn" onClick={()=>setShowInfo(true)} title="View full data disclaimer">ⓘ</button></div>)}
...
<button className="modal-close" onClick={()=>setShowPrivacy(false)} title="Close">✕</button>
<div className="modal-icon">🔒</div>
<h3 className="modal-heading">Privacy Notice</h3>
<p className="modal-body">{PRIVACY_TEXT}</p>
<button className="modal-btn" onClick={()=>setShowPrivacy(false)}>I understand — Close</button>
</div>
</div>
)}
{showInfo&&(
...
<button className="modal-close" onClick={()=>setShowInfo(false)} title="Close">✕</button>
<h3 className="modal-heading" style={{marginBottom:16}}>Data Disclaimer &amp; Limitations</h3>
{INFO_SECTIONS.map((s,i)=>(
<div key={i} className="info-section">
<div className="info-section-title">{i+1}. {s.title}</div>
<p className="info-section-body">{s.body}</p>
</div>
))}
<button className="modal-btn" onClick={()=>setShowInfo(false)}>Close</button>
</div>
</div>
)}
</div>);}
