...
s2=ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED').filterBounds(bbox).filterDate('2025-09-01','2026-03-16').filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',50)).sort('system:time_start',False).first()
...
for ft in result['features']:
    ...
    if v is None or v<0.02:continue
    lon,lat=ft['geometry']['coordinates']
    sev='High' if v>0.15 else 'Medium' if v>0.05 else 'Low'
    features.append({'type':'Feature','geometry':{'type':'Point','coordinates':[lon,lat]},'properties':{'sfabi':round(v,4),'ndci':round(v*0.8,4),'severity':sev,'lat':round(lat,4),'lon':round(lon,4),'date':datetime.now().strftime('%Y-%m-%d')}})
geojson={'type':'FeatureCollection','features':features,'metadata':{'generated_at':datetime.now().isoformat(),'source':'Real Sentinel-2 GEE','total_cells':len(features)}}
open('data/indices/bloom_heatmap_latest.geojson','w').write(json.dumps(geojson))
