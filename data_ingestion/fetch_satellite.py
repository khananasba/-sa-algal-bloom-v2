...
def authenticate():
    ...
    On Render: reads GEE_SERVICE_ACCOUNT_JSON env var (full JSON as a string).
    Locally:   falls back to default `earthengine authenticate` credentials.
    ...
    try:
        ...
        if sa_json:
            key_data    = json_lib.loads(sa_json)
            credentials = ee.ServiceAccountCredentials(
                key_data['client_email'],
                key_data=json_lib.dumps(key_data)
            )
            ee.Initialize(credentials, project='smart-464108')
            print('GEE authenticated via service account:', key_data['client_email'])
        else:
...
def fetch_and_write():
    """Fetch Sentinel-2 SFABI, write GeoJSON. Returns feature count or 0 on failure."""
    try:
        ...
        print(f'GEE fetch/write failed: {e}')
        return 0


if __name__ == '__main__':
    if not authenticate():
        raise SystemExit('GEE authentication failed — cannot proceed')
    count = fetch_and_write()
    if count == 0:
        raise SystemExit('GEE returned 0 features — heatmap not updated')
else:
    # When imported or run as a subprocess by refresh_data.py:
    # authenticate and run immediately (module-level execution expected by orchestrator)
    if not authenticate():
        raise SystemExit('GEE authentication failed')
    fetch_and_write()
